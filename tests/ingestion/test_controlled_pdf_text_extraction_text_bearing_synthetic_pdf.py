...
SYNTHETIC_TEXT = "SYNTHETIC PDF TEXT FOR CONTROLLED PARSER TEST ONLY"
MAX_PREVIEW_CHARACTERS = 1000


def _write_text_bearing_synthetic_pdf(tmp_path: Path) -> Path:
    pdf_path = tmp_path / "text-bearing-synthetic-parser-test.pdf"
    writer = PdfWriter()
    ...
    content_stream.set_data(
        f"BT /F1 12 Tf 72 720 Td ({SYNTHETIC_TEXT}) Tj ET".encode("ascii")
    )
...
def test_controlled_text_bearing_synthetic_pdf_returns_extracted_result(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    pdf_path = _write_text_bearing_synthetic_pdf(tmp_path)
    fixture = ControlledRealAssetFixtureItem(
        fixture_id="fixture-product-spec",
        source_label="synthetic text parser fixture",
        ...
        notes="synthetic text extraction only",
    )
    ...
    assert result.text_length == len(result.text_preview)
    assert result.text_preview == SYNTHETIC_TEXT
    assert len(result.text_preview) <= MAX_PREVIEW_CHARACTERS
